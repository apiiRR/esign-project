            ...
            <LayoutAdmin>
                <div className="space-y-6">
                    <div className="flex items-end justify-between gap-4">
                        ...
                        {isArchive ? (
                            <Link href="/admin/surat/create/arsip" className="inline-flex items-center rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white"><Plus className="mr-2 h-4 w-4" />Upload Scan</Link>
                        ) : isIncomingExternal ? (
                            <Link href="/admin/surat/create/masuk-eksternal" className="inline-flex items-center rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white"><Plus className="mr-2 h-4 w-4" />Input Surat Eksternal</Link>
                        ) : (
                            <Link href={`/admin/surat/create/${type === "outgoing" ? "keluar" : "internal"}`} className="inline-flex items-center rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white"><Plus className="mr-2 h-4 w-4" />Buat Surat</Link>
                        )}
                    </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {(letters.data || []).map((letter) => (
                                        <tr key={letter.id}>
                                            <td className="px-5 py-4"><div className="font-semibold text-gray-950">{letter.subject}</div><div className="text-xs text-gray-500">{letter.reference} · {letter.letter_number || "-"}</div></td>
